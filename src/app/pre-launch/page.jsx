...
function CountdownDisplay({ targetDate }) {
  ...
  if (launched) {
    return (
      <div className="flex items-center space-x-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 rounded-xl text-emerald-400 text-xs font-bold">
      ...
      {[
        ...
        <React.Fragment key={label}>
          <div className="text-center">
            <div className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 min-w-[52px]">
              <span className="text-xl font-extrabold text-white tabular-nums block leading-none">{val}</span>
              <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mt-0.5 block">{label}</span>
            </div>
          </div>
...
export default function PreLaunchPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950">
      ...
      {/* ── HERO BANNER ── */}
      <section className="relative py-28 px-4 sm:px-6 lg:px-8 overflow-hidden bg-slate-950 text-white">
        ...
        <div className="relative max-w-4xl mx-auto text-center z-10 space-y-6">
          <div className="inline-flex items-center space-x-2 px-4 py-2 bg-gold-500/10 border border-gold-500/20 rounded-full text-gold-400 text-[10px] font-bold uppercase tracking-widest">
            <Rocket className="h-3.5 w-3.5 animate-bounce" />
            <span>Coming Soon</span>
          </div>
          <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight leading-tight">
            Upcoming <span className="bg-clip-text text-transparent bg-gradient-to-r from-gold-400 via-amber-400 to-gold-500">Pre-Launch</span>
            <br />Properties
          </h1>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 pt-2">
            ...
            <a
              ...
              className="inline-flex items-center justify-center space-x-2 px-8 py-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold rounded-xl text-sm transition-all"
            >
      ...
      {/* ── LISTINGS ── */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-slate-900/60 flex-grow">
        <div className="max-w-7xl mx-auto space-y-10">

          {loading ? (
            // Skeleton loaders
            [1, 2, 3].map(i => (
              <div key={i} className="animate-pulse bg-slate-800/40 border border-slate-700/30 rounded-3xl overflow-hidden flex flex-col lg:flex-row h-72" />
            ))
          ) : error ? (
            <div className="text-center py-24 text-slate-400">
              <Rocket className="h-12 w-12 mx-auto mb-4 text-slate-700" />
              <p className="font-bold text-rose-400 mb-2">Unable to connect to server</p>
            ...
            items.map((item, index) => {
              const catColor = CATEGORY_COLORS[item.category] || 'text-gold-400 bg-gold-500/10 border-gold-500/20';
              const isEven = index % 2 === 0;

              return (
                <div
                  key={item._id}
                  className="group bg-slate-800/30 border border-slate-700/50 hover:border-gold-500/30 rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-gold-500/5 flex flex-col lg:flex-row"
                >
                  ...
                  <div className={`relative lg:w-[42%] h-64 lg:h-auto shrink-0 overflow-hidden ${isEven ? 'lg:order-1' : 'lg:order-2'}`}>
                    ...
                    <span className={`absolute top-5 left-5 px-3 py-1 rounded-lg text-[10px] font-extrabold uppercase tracking-widest border ${catColor}`}>
                      {item.category}
                    </span>
                  </div>
                  ...
                  <div className={`flex flex-col justify-between p-8 lg:p-10 flex-grow ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                    ...
                    <div className="space-y-5">
                      ...
                      <h2 className="text-2xl sm:text-3xl font-extrabold text-white group-hover:text-gold-400 transition-colors leading-snug">
                        {item.heading}
                      </h2>
                    ...
                    <div className="mt-8 space-y-5">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <a
                          href="tel:+919000000000"
                          className="inline-flex items-center justify-center space-x-2 px-6 py-3 bg-slate-700/60 hover:bg-slate-700 border border-slate-600 text-slate-200 font-bold rounded-xl text-xs transition-all"
                        >
        ...
        <div className="max-w-3xl mx-auto text-center space-y-5">
          ...
          >
            <input
              ...
              className="flex-grow px-5 py-3.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-xs placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-gold-500 focus:border-gold-500 transition-all"
            />
            <button
              type="submit"
